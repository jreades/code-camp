...
Now is when you either start Python on your own computer (usually from the Terminal/Command Prompt) or mosy over to Google Collab (to run Python in the cloud). If you don't know how to do either, please visit the [install](../setup/install.qmd) and [no install](../setup/no_install.qmd) pages.
...
If you got the result `2`, you are using Python version 2 and will need to check (and change) your installation of Python (go back to the [Python Setup page](../setup/install.qmd), or try posting a query on Slack for help).  
...
*Remember*: copy (clicking on the clipboard icon on the right) and paste the code in the cell above in whatever Python environment you are using (either [online](../setup/no_install.qmd) or on [your machine](../setup/install.qmd)) and 'run' the code cell and get the answer.
